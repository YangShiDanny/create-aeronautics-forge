...
public class SteeringWheelHandler extends BlockHoldInteraction {
    ...
    public void stop() {
        ...
        dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new SteeringWheelPacket(true, effectiveAngle, this.getInteractionPos()));
        super.stop();
    }
    ...
    public void setTargetAngle(final float targetAngle) {
        if (updated) {
            dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new SteeringWheelPacket(false, targetAngle, this.getInteractionPos()));
