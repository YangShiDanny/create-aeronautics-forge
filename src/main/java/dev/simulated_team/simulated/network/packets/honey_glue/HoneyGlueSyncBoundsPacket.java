...
public record HoneyGlueSyncBoundsPacket(AABB bounds, int honeyGlueId, UUID uuid) implements CustomPacketPayload {
    ...
    public static StreamCodec<FriendlyByteBuf, HoneyGlueSyncBoundsPacket> CODEC = StreamCodec.of(HoneyGlueSyncBoundsPacket::writeToBuf, HoneyGlueSyncBoundsPacket::readFromBuf);

    public static void writeToBuf(final FriendlyByteBuf buf, final HoneyGlueSyncBoundsPacket packet) {
    ...
    public static HoneyGlueSyncBoundsPacket readFromBuf(final FriendlyByteBuf buf) {
    ...
    public void handle(final ClientPacketContext context) {
        ...
        if (this.uuid != null && this.uuid.equals(context.getPlayer().getUUID()))
            return;
