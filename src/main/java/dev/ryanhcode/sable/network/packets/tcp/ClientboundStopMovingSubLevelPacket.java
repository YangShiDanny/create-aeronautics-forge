...
public record ClientboundStopMovingSubLevelPacket(long plotCoordinate) implements SableTCPPacket {
    public static void encode(final FriendlyByteBuf buf, final ClientboundStopMovingSubLevelPacket msg) {
        msg.write(buf);
    }

    public static ClientboundStopMovingSubLevelPacket decode(final FriendlyByteBuf buf) {
        return ClientboundStopMovingSubLevelPacket.read(buf);
    }
