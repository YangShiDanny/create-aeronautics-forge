...
//        this.editor2.init(); //we no longer init like this...
    ...
    public void onClose() {
        ...
        dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new TypewriterDisconnectUser(this.clientBe.getBlockPos()));
        super.onClose();
    }
    ...
    public void sendNewKeys(final boolean clearServer) {
        ...
        // save keys!
        dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new TypewriterKeySavePacket(this.newEntries, this.clientBe.getBlockPos(), clearServer));
    }
