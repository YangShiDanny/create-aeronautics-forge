...
public class ActiveSableCompanion implements SableCompanion {
    ...
    @Override
    public  SubLevel getContaining(final Level level, final int chunkX, final int chunkZ) {
    ...
    @Override
    public  SubLevel getContaining(final Level level, final ChunkPos chunkPos) {
        return this.getContaining(level, chunkPos.x, chunkPos.z);
    }

    @Override
    public  SubLevel getContaining(final Level level, final SectionPos pos) {
        return this.getContaining(level, pos.getX(), pos.getZ());
    }

    @Override
    public  SubLevel getContaining(final Level level, final Vec3i pos) {
        return this.getContaining(level, pos.getX() >> SectionPos.SECTION_BITS, pos.getZ() >> SectionPos.SECTION_BITS);
    }

    @Override
    public  SubLevel getContaining(final Level level, final Position pos) {
        return this.getContaining(level, Mth.floor(pos.x()) >> SectionPos.SECTION_BITS, Mth.floor(pos.z()) >> SectionPos.SECTION_BITS);
    }

    @Override
    public  SubLevel getContaining(final Level level, final Vector3dc pos) {
        return this.getContaining(level, Mth.floor(pos.x()) >> SectionPos.SECTION_BITS, Mth.floor(pos.z()) >> SectionPos.SECTION_BITS);
    }

    @Override
    public  SubLevel getContaining(final Level level, final double blockX, final double blockZ) {
    ...
    @Override
    public  ClientSubLevel getContainingClient(final int chunkX, final int chunkZ) {
        return (ClientSubLevel) this.getContaining(SableDistUtil.getClientLevel(), chunkX, chunkZ);
    }

    @Override
    public  ClientSubLevel getContainingClient(final ChunkPos chunkPos) {
        return (ClientSubLevel) this.getContaining(SableDistUtil.getClientLevel(), chunkPos.x, chunkPos.z);
    }

    @Override
    public  ClientSubLevel getContainingClient(final Position pos) {
    ...
    @Override
    public  ClientSubLevel getContainingClient(final Vector3dc pos) {
        return (ClientSubLevel) this.getContaining(SableDistUtil.getClientLevel(), Mth.floor(pos.x()) >> SectionPos.SECTION_BITS, Mth.floor(pos.z()) >> SectionPos.SECTION_BITS);
    }

    @Override
    public  ClientSubLevel getContainingClient(final SectionPos pos) {
        return (ClientSubLevel) this.getContaining(SableDistUtil.getClientLevel(), pos.x(), pos.z());
    }

    @Override
    public  ClientSubLevel getContainingClient(final Vec3i pos) {
    ...
    @Override
    public  ClientSubLevel getContainingClient(final double blockX, final double blockZ) {
    ...
    @Override
    public  <T, S extends SubLevelAccess> T runIncludingSubLevels(final Level level, final Vec3 origin, final boolean shouldCheckOrigin,  final S subLevel, final BiFunction<S, BlockPos, T> converter) {
        return this.runIncludingSubLevels(level, (Position) origin, shouldCheckOrigin, subLevel, converter);
    }

    @SuppressWarnings("unchecked")
    @Override
    public  <T, S extends SubLevelAccess> T runIncludingSubLevels(final Level level, final Position origin, final boolean shouldCheckOrigin,  final S subLevel, final BiFunction<S, BlockPos, T> converter) {
    ...
    @Override
    public <S extends SubLevelAccess> boolean findIncludingSubLevels(final Level level, final Vec3 origin, final boolean shouldCheckOrigin,  final S subLevel, final BiFunction<S, BlockPos, Boolean> converter) {
        return this.findIncludingSubLevels(level, (Position) origin, shouldCheckOrigin, subLevel, converter);
    }

    @Override
    public <S extends SubLevelAccess> boolean findIncludingSubLevels(final Level level, final Position origin, final boolean shouldCheckOrigin,  final S subLevel, final BiFunction<S, BlockPos, Boolean> converter) {
    ...
    @Override
    public  SubLevel getLastTrackingSubLevel(final Entity entity) {
    ...
    @Override
    public  SubLevel getVehicleSubLevel(final Entity entity) {
    ...
    @Override
    public  Vector3d getFeetPos(final Entity entity, final float distanceDown) {
