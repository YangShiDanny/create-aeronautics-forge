...
public class ClientHandleHandler extends BlockHoldInteraction {
    ...
    public boolean activeTick(final Level level, final LocalPlayer player) {
        ...
        if (!crouchingOrFlying) {
            ...
            dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new UpdatePlayerUsingHandlePacket(-1, false, interactionPos));
    ...
    public void clientTick(final Level level, final LocalPlayer player) {
        ...
        if (player.getMainHandItem().is(AllItems.WRENCH.get()) || player.getOffhandItem().is(AllItems.WRENCH.get())) {
            return;
        }
        if (!HandleBlock.canInteractWithHandle(player))
            return;
    ...
    private void sendUpdate(final boolean remove) {
        dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new UpdatePlayerUsingHandlePacket(remove ? -1 : this.desiredRange, remove, this.getInteractionPos()));
    }
}
