...
import dev.simulated_team.simulated.content.physics_staff.PhysicsStaffItem;import dev.simulated_team.simulated.content.items.spring.SpringItem;
...
public class SimItems {
    ...
    // [1.20.1 port] NeoForge 1.21 独有标签（Tags.Items.STRINGS / ENCHANTABLES、ItemTags.DURABILITY）
    // 在 1.20.1 Forge 中不存在，按 forge: 命名空间自建 TagKey 以维持配方意图（运行时需 datagen 填充）。
    private static final TagKey<Item> FORGE_STRINGS = TagKey.create(Registries.ITEM, new ResourceLocation("forge", "strings"));
    private static final TagKey<Item> FORGE_ENCHANTABLES = TagKey.create(Registries.ITEM, new ResourceLocation("forge", "enchantables"));
    private static final TagKey<Item> FORGE_DURABILITY = TagKey.create(Registries.ITEM, new ResourceLocation("forge", "durability"));
    ...
    public static ItemEntry<RopeItem> ROPE_COUPLING = REGISTRATE
            ...
            .recipe((ctx, prov) -> ShapedRecipeBuilder.shaped(RecipeCategory.MISC, ctx.get(), 1)
                    ...
                    .pattern("NSN")
                    .pattern(" S ")
                    .define('S', FORGE_STRINGS)
                    .define('N', Tags.Items.NUGGETS_IRON)
                    .unlockedBy("has_ingredient", RegistrateRecipeProvider.has(FORGE_STRINGS))
                    .save(prov))
            .register();
    ...
    static { REGISTRATE.addExtraItem(new ResourceLocation("minecraft", "slime_ball")); }

    public static final ItemEntry<HoneyGlueItem> HONEY_GLUE =
            REGISTRATE.item("honey_glue", HoneyGlueItem::new)
                    .properties(p -> p.stacksTo(1)
                            .durability(100))
                    .tag(FORGE_DURABILITY)
                    .register();

    // [1.20.1 port] physics_staff restored — see build.gradle (exclusions removed)
    ...
    public static final ItemEntry<PlungerLauncherItem> PLUNGER_LAUNCHER =
            REGISTRATE.item("plunger_launcher", PlungerLauncherItem::new)
                    ...
                    .model(AssetLookup.itemModelWithPartials())
                    .tag(FORGE_ENCHANTABLES, FORGE_DURABILITY)
                    .register();
