...
import foundry.veil.api.client.render.PoseStack;
...
import dev.simulated_team.simulated.libs.minecraft.client.DeltaTracker;
...
public class EndSeaShadowRenderer {
    ...
    public static void renderShadowMap(final VeilRenderLevelStageEvent.Stage stage, final LevelRenderer levelRenderer, final MultiBufferSource.BufferSource bufferSource, final PoseStack matrixStack, final Matrix4fc frustumMatrix, final Matrix4fc projectionMatrix, final int renderTick, final DeltaTracker deltaTracker, final Camera camera, final Frustum frustum) {
    ...
    public static void renderVoidAnchors(final Camera camera) {
        ...
        final BufferBuilder builder = Tesselator.getInstance().begin(DefaultVertexFormat.POSITION_TEX_COLOR, VertexFormat.Mode.QUADS);
        ...
        for (final Vector3dc voidAnchor : voidAnchors) {
            ...
            builder.vertex(pose, -size, 0, -size).uv(0.0f, 0.0f);
            builder.vertex(pose, size, 0, -size).uv(1.0f, 0.0f);
            builder.vertex(pose, size, 0, size).uv(1.0f, 1.0f);
            builder.vertex(pose, -size, 0, size).uv(0.0f, 1.0f);
        }
