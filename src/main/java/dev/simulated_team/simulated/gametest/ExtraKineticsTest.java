...
public class ExtraKineticsTest {
    ...
    public static void torsionSpring(final GameTestHelper helper) {
        helper.startSequence()
                ...
                .thenExecuteAfter(15, () -> SimulatedGameTestHelper.assertBlockEntityData(helper, new BlockPos(2, 2, 3),
                        (TorsionSpringBlockEntity be) -> Math.abs(be.getAngle()) == 90,
                        () -> "Expected 90 degrees, got %.0f".formatted(Math.abs(((TorsionSpringBlockEntity) helper.getBlockEntity(new BlockPos(2, 2, 3))).getAngle()))))
                .thenExecuteAfter(1, () -> helper.setBlock(1, 2, 2, helper.getBlockState(new BlockPos(1, 2, 2)).setValue(LeverBlock.POWERED, true)))
                .thenExecuteAfter(15, () -> SimulatedGameTestHelper.assertBlockEntityData(helper, new BlockPos(2, 2, 3),
                        (TorsionSpringBlockEntity be) -> be.getAngle() == 0,
