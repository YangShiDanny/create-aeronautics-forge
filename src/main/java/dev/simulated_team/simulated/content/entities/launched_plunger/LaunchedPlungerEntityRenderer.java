...
public class LaunchedPlungerEntityRenderer extends EntityRenderer<LaunchedPlungerEntity> {
    ...
    public void render(final LaunchedPlungerEntity entity, final float f, final float pt, final PoseStack poseStack, final MultiBufferSource multiBufferSource, final int light) {
        ...
        if (entity.isPlunged()) {
            ...
            selfNormal = entity.getViewVector(-Mth.lerp(pt, entity.yRotO, entity.getYRot())).reverse();
            perpendicularNormal = entity.getViewVector(-Mth.lerp(pt, entity.yRotO, entity.getYRot()) - 90).reverse();
        }
        ...
        if (other != null) {
            ...
            if (other.isPlunged()) {
                ...
                otherNormal = other.getViewVector(-Mth.lerp(pt, other.yRotO, other.getYRot())).reverse();
            }
            ...
            if (entity.getData(LaunchedPlungerEntity.OTHER_PLUNGER).isEmpty() && owner == Minecraft.getInstance().player && Minecraft.getInstance().options.getCameraType().isFirstPerson()) {
                ...
                if (owner instanceof final AbstractClientPlayer player && entity.getData(LaunchedPlungerEntity.OTHER_PLUNGER).isEmpty()) {
                    ...
                    final float bodyDifference = Math.abs(headYDirection - player.getYRot()) / 50f;
                    ...
                    headYDirection = Mth.lerp(lookDelta, headYDirection, player.getYRot());
                    final Vec3 viewDirection = player.getViewVector(headYDirection);
                    final Vec3 handDirection = player.getViewVector(headYDirection + 90.0f);
    ...
    public static void renderRope(final List<Vec3> positions, final MultiBufferSource multiBufferSource, final BlockAndTintGetter level, final PoseStack poseStack) {
        ...
        for (int i = 0; i < splinePoints.size() - 1; i++) {
            ...
            builder.vertex((float) (nextX - origin.x() + POS.x), (float) (nextY - origin.y() + POS.y), (float) (nextZ - origin.z() + POS.z));
            builder.uv(0, nextV);
            builder.uv2(lightEnd);
            builder.normal(NEXT_NORMAL.x, NEXT_NORMAL.y, NEXT_NORMAL.z);

            ORIENTATION.transform(POS.set(-cableRadius, -cableRadius, 0));
            builder.vertex((float) (x - origin.x() + POS.x), (float) (y - origin.y() + POS.y), (float) (z - origin.z() + POS.z));
            builder.uv(0, v);
            builder.uv2(lightStart);
