...
public record RopeBreakPacket(UUID uuid) implements CustomPacketPayload {
    ...
    public static StreamCodec<FriendlyByteBuf, RopeBreakPacket> CODEC = StreamCodec.composite(
            BackportCodecs.UUID, RopeBreakPacket::uuid,
            RopeBreakPacket::new
    );
    ...
    public void handle(final ServerPacketContext ctx) {
        ...
        if (strand != null) {
            ...
            holder.destroyRope(player, null, !player.isCreative());
        }
    }
}
