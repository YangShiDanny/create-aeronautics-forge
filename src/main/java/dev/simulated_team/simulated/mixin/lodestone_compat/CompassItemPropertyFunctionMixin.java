...
public abstract class CompassItemPropertyFunctionMixin {
	...
	@WrapMethod(remap = false, method = "m_234954_")
    private float simulated$prioritizeID(final ItemStack stack, final ClientLevel level, final int seed, final Entity entity, final Operation<Float> original) {
        if (DataComponentType.has(stack, SimDataComponents.LODESTONE_COMPASS_SUBLEVEL_TRACKER)) {
	        final UUID trackerID = DataComponentType.get(stack, SimDataComponents.LODESTONE_COMPASS_SUBLEVEL_TRACKER);
