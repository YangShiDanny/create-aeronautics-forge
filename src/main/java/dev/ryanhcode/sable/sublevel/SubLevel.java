...
public abstract class SubLevel implements SubLevelAccess {
    ...
    protected SubLevel(final Level level, final int plotX, final int plotY, final Pose3d pose) {
        ...
        // NOTE: globalBoundsTransform intentionally left as identity (matches upstream neo
        // behaviour). Plot chunk holders are registered with LOCAL chunk positions
        // (see ServerLevelPlot#addChunkHolder), so plot bounds are already in local
        // 0..plotSize space; applying a multi-million translation here would push
        // them to negative millions and (after pose rotation) corrupt the Y axis.
    }
