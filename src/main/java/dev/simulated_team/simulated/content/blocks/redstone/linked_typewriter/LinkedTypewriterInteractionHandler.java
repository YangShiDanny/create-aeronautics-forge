...
import dev.simulated_team.simulated.libs.minecraft.world.item.component.ItemContainerContents;
...
public class LinkedTypewriterInteractionHandler {
    ...
    public static void tick() {
        ...
        if (getMode() == Mode.ACTIVE &&
            ...
            dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new TypewriterDisconnectUser(be.getBlockPos()));
            ...
            if (be != null && !be.isRemoved()) {
                ...
                if (frequency != null) {
                    ...
                    if (action != GLFW.GLFW_REPEAT) {
                        dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new TypewriterKeyInteractionPacket(be.getBlockPos(), key, scanCode, action));
                    }
    ...
    public static void sendLinkedControllerData(final Level level, final BlockPos blockPos, final ItemStack item) {
        ...
        final ItemContainerContents linkedControllerData = null;
        ...
        dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new TypewriterKeySavePacket(newKeyBindings, blockPos, false));
    }
