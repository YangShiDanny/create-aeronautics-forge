...
public class MassTracker implements MassData {
    ...
    public static MassTracker build(final BlockGetter blockGetter, final BoundingBox3ic bounds) {
        ...
        // Defensive guard: an absurdly large bounds (e.g. a whole plot
        // region passed in by mistake) would make the double triple-loop
        // below iterate billions of blocks and freeze the server thread for
        // minutes. Bail out with an empty tracker in that case.
        final long volumeEstimate = (long) (bounds.maxX() - bounds.minX() + 1)
                * (bounds.maxY() - bounds.minY() + 1)
                * (long) (bounds.maxZ() - bounds.minZ() + 1);
        if (volumeEstimate > 25_000_000L) {
    ...
    private static Matrix3d addBlockInertia(final Vector3d blockPos, final double blockMass, final Matrix3d dest, final  Vec3 blockInertia) {
     ...
     */
    public void addBlockMass(final BlockGetter blockGetter, final BlockState state, final BlockPos blockPos, final double blockMass, final  Vec3 blockInertia) {
