        ...
        if (clientHit instanceof final BlockHitResult hit && hit.getType() != HitResult.Type.MISS) {
            ...
            if (!canSupportGlue(level, pos, normal)) {
                if (this.firstPos == null) {
                    ...
                    player.swing(hand);
                    dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new PlaceMergingGluePacket(this.firstPos, pos, this.firstDirection, normal, hand));
                    this.reset(false);
                    return true;
                }
