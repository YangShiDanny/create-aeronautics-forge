...
public class PlungerLauncherItem extends Item implements CustomArmPoseItem {
    ...
    public InteractionResultHolder<ItemStack> use(final Level level, final Player player, final InteractionHand interactionHand) {
        ...
        org.apache.logging.log4j.LogManager.getLogger(PlungerLauncherItem.class).info("[PlungerLink] use: plunger={} removed={} newPlunger={}", plunger != null ? plunger.getId() : "null", plunger != null && plunger.isRemoved(), newPlunger.getId());
        if (plunger == null || plunger.isRemoved()) {
                ...
                // [1.20.1 port] 立即把双方实体数据(含 OTHER_PLUNGER_ID / IS_FIRST)推送给持有发射器的玩家，
                // 绕过 SynchedEntityData 对已存在实体更新偶发不广播 dirty 的问题，确保客户端先发射的皮搋子 other 引用及时同步，否则绳子跟随玩家移动。
                if (player instanceof final ServerPlayer sp) {
                    sp.connection.send(new net.minecraft.network.protocol.game.ClientboundSetEntityDataPacket(plunger.getId(), plunger.getEntityData().getNonDefaultValues()));
                    sp.connection.send(new net.minecraft.network.protocol.game.ClientboundSetEntityDataPacket(newPlunger.getId(), newPlunger.getEntityData().getNonDefaultValues()));
                }
            ...
            // send shoot packet
            dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.getPlayer((ServerPlayer) player).sendPacket(new PlungerLauncherShootPacket(interactionHand));
            // todo: send to all tracking players for playing (to be implemented) external animation
//            dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.tracking(player).sendPacket(new PlungerLauncherShootPacket(interactionHand));

            if (!BacktankUtil.canAbsorbDamage(player, maxUses()))
                heldStack.hurtAndBreak(1, player, (LivingEntity p) -> {});
        } else {
    ...
    public  BarrelAndCorrectionInfo getCorrectionInfo(final Player player, final InteractionHand interactionHand) {
    ...
    public void appendHoverText(final ItemStack stack, @Nullable final Level level, final List<Component> tooltip, final TooltipFlag flag) {
        ...
        SimLang.translate("plunger_launcher.bug_warn").color(0xFF5555).forGoggles(tooltip);
    }
}
