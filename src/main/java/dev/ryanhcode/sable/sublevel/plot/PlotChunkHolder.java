...
public class PlotChunkHolder extends ChunkHolder {
    ...
    public PlotChunkHolder(final LevelChunk chunk, final ChunkPos pos, final LevelHeightAccessor levelHeightAccessor, final LevelLightEngine levelLightEngine, final LevelChangeListener levelChangeListener, final PlayerProvider playerProvider) {
        ...
        final Either<LevelChunk, ChunkHolder.ChunkLoadingFailure> loadedChunk = Either.left(chunk);
        this.tickingChunkFuture = CompletableFuture.completedFuture(loadedChunk);
        this.entityTickingChunkFuture = CompletableFuture.completedFuture(loadedChunk);
        this.fullChunkFuture = CompletableFuture.completedFuture(loadedChunk);
     ...
     */
    public  HeatDataChunkSection getHeatSection(final int y) {
