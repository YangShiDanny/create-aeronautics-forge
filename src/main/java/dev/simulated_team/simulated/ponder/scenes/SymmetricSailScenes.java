...
public class SymmetricSailScenes {
    ...
    public static void linelessTextbox(final CreateSceneBuilder scene, final String text, final Vec3 position, final PonderPalette color, final int ticks) {
        ...
        // 防御性处理：TextWindowElementExtension 由 TextWindowElementMixin 在客户端注入。
        // datagen（无完整客户端）等环境下 mixin 可能未生效，此时直接强转会 ClassCastException。
        // 用 instanceof 兜底，未生效时跳过 hidePointer（仅场景里多显示一个小箭头，不影响功能）。
        if (textWindowElement instanceof TextWindowElementExtension ext) {
            ext.simulated$hidePointer();
        }
