...
public class DiagramForceGroupToggle extends AbstractWidget {
    ...
    public DiagramForceGroupToggle(final DiagramScreen diagramScreen, final ForceGroup forceGroup, final int x, final int y) {
        ...
        this.groupId = ForceGroups.REGISTRY.entrySet().stream().filter(e -> e.getValue() == forceGroup).map(java.util.Map.Entry::getKey).findFirst().orElse(null);
    ...
    public void updateForceState( final DiagramDataPacket serverData) {
        if (serverData == null) {
            this.forceCount = 0;
            return;
        }
