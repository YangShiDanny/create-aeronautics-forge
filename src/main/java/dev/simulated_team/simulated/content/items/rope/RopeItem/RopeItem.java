...
public class RopeItem extends Item {
    ...
    public InteractionResult useOn(final UseOnContext context) {
        ...
        if (player != null && player.isShiftKeyDown()) {
            DataComponentType.remove(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION);
            return InteractionResult.SUCCESS;
        }

        if (validLocation) {
            if (DataComponentType.has(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION)) {

                if (!level.isClientSide) {
                    if (!this.attachRope(level, DataComponentType.get(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION), clickedPos, !player.isCreative())) {
                        // failure to connect
                        DataComponentType.remove(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION);
                        return InteractionResult.SUCCESS;
                    } else {
            ...
            DataComponentType.set(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION, clickedPos);
            return InteractionResult.SUCCESS;
        }
