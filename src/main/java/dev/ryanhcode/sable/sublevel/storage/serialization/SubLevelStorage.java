...
public class SubLevelStorage implements AutoCloseable {
    ...
    private static  String getFileName(final ChunkPos chunkPos) {
        return "r." + chunkPos.getRegionX() + "." + chunkPos.getRegionZ();
    }

    private static  String getFileName(final ChunkPos chunkPos, final int index) {
    ...
    private SubLevelRegionFile getRegionFile(final ChunkPos chunkPos) throws IOException {
        ...
        if (this.regionCache.size() >= MAX_CACHE_SIZE) {
            // Long2ObjectLinkedOpenHashMap has no remove(int); remove the least-recently-used entry by key
            this.regionCache.remove(this.regionCache.lastLongKey()).close();
        }
    ...
    private SubLevelStorageFile getRegionStorageFile(final ChunkPos chunkPos, final int index) throws IOException {
        ...
        if (this.storageCache.size() >= MAX_CACHE_SIZE) {
            // Long2ObjectLinkedOpenHashMap has no remove(int); remove the least-recently-used entry by key
            this.storageCache.remove(this.storageCache.lastLongKey()).close();
        }
     ...
     */
    private  Path getExternalPath(final ChunkPos chunkPos) {
     ...
     */
    private  Path getExternalPath(final ChunkPos chunkPos, final int index) {
     ...
     * @return the storage file for the chunk position
     */
    private  Path getPath(final ChunkPos chunkPos) {
     ...
     * @return the path to the storage file
     */
    private  Path getPath(final ChunkPos chunkPos, final int index) {
