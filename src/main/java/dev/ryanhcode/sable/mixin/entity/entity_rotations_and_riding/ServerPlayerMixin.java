...
public abstract class ServerPlayerMixin extends Player {
    ...
    // 1.20.1: refMap 未对本 mixin 生成条目（编译期 AP 未解析），改用 SRG 名直写 + remap=false。
    // m_7998_ = ServerPlayer.startRiding；m_9774_ = ServerGamePacketListenerImpl.teleport(DDDFF)V（均经 1.20.1 字节码实证）。
    @WrapOperation(method = "m_7998_", at = @At(value = "INVOKE", target = "Lnet/minecraft/server/network/ServerGamePacketListenerImpl;m_9774_(DDDFF)V", remap = false), remap = false)
