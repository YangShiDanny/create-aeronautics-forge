...
public interface NavigationTarget {
    ...
    // [1.20.1 port] 原版用 TARGET 数据组件判定手持物是否为导航目标，该组件由 NeoForge 1.21 的
    // ModifyDefaultComponentsEvent 挂载；Forge 1.20.1 无此事件，故改为按物品类型反查（见 SimulatedRegistrate）。
    static NavigationTarget ofStack(final ItemStack itemStack) {
        return SimulatedRegistrate.getNavigationTarget(itemStack);
    }
}
