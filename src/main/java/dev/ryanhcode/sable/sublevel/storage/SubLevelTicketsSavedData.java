    ...
    private static SubLevelTicketsSavedData load(final ServerLevel level, final CompoundTag tag) {
        ...
        for (int i = 0; i < ticketInfos.size(); i++) {
            ...
            if (infoTag.contains("pointer")) {
                pointer = GlobalSavedSubLevelPointer.CODEC.parse(NbtOps.INSTANCE, infoTag.get("pointer")).result().orElse(null);
            }
    ...
    public  CompoundTag save(final CompoundTag compoundTag) {
        ...
        for (final Map.Entry<UUID, SubLevelTicketInfo> entry : allTickets.entrySet()) {
            ...
            if (!entriesTag.isEmpty()) {
                if (pointer != null) {
                    infoTag.put("pointer", GlobalSavedSubLevelPointer.CODEC.encodeStart(NbtOps.INSTANCE, pointer).getOrThrow(true, msg -> {}));
                } else {
