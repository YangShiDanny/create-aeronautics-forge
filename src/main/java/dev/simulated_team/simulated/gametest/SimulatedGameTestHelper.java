...
public class SimulatedGameTestHelper {
    ...
    public static <T extends BlockEntity> void assertBlockEntityData(final GameTestHelper helper, final BlockPos pos, final Predicate<T> predicate, final Supplier<String> message) {
    ...
    public static <T extends KineticBlockEntity> void assertKineticsSpeed(final GameTestHelper helper, final BlockPos pos, final ToDoubleFunction<T> speed, final double delta) {
        final T be = (T) helper.getBlockEntity(pos);
        if (!(Math.abs(Math.abs(be.getSpeed()) - Math.abs(speed.applyAsDouble(be))) < delta)) {
            throw new GameTestAssertPosException("Expected %.2f speed, got %.2f".formatted(Math.abs(speed.applyAsDouble(be)), Math.abs(be.getSpeed())), helper.absolutePos(pos), pos, helper.getTick());
        }
    }
