...
public class SableCommand {
                ...
                .then(Commands.literal("info").then(Commands.argument("sub_level", SubLevelArgumentType.subLevels()).executes(ctx -> {
                    ...
                    for (final ServerSubLevel subLevel : subLevels) {
                        ...
                        source.sendSuccess(() -> {
                            ...
                            component.setStyle(Style.EMPTY.withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, new Formatter().format(Locale.ROOT, "/execute in %s run tp @s %.2f %.2f %.2f", dimension, pos.x(), pos.y(), pos.z()).toString())).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, fileId)).withColor(ChatFormatting.GRAY));
                            return component;
                        }, false);
                        source.sendSuccess(() -> {
                            final Vector3dc pos = pose.position();
    ...
    private static int executeEnableGizmoCommand(final CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        SableTCPPackets.sendToPlayer(player, new ClientboundEnterGizmoPacket());
        return 1;
    }
