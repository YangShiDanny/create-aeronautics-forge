...
public class HoneyGlueEntity extends Entity implements SpecialEntityItemRequirement {
    ...
    public void setBoundsAndSync(final AABB bounds,  final Player player) {
        final Level level = this.level();
    ...
    public void syncBounds( final Player player) {
        dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.tracking(this).sendPacket(new HoneyGlueSyncBoundsPacket(this.getBoundingBox(), this.getId(), player != null ? player.getUUID() : null));
    }
