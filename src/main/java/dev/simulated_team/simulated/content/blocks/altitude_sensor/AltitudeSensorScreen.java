...
public class AltitudeSensorScreen extends AbstractSimiScreen {
    ...
    protected void renderWindow(final GuiGraphics graphics, final int mouseX, final int mouseY, final float partialTicks) {
        ...
        bufferbuilder.vertex(ps.last().pose(), px2, py1, 0.0f).uv(uvx2, uvy1).color(255, 255, 255, 255).endVertex();
        bufferbuilder.vertex(ps.last().pose(), px1, py1, 0.0f).uv(uvx1, uvy1).color(255, 255, 255, 255).endVertex();
        bufferbuilder.vertex(ps.last().pose(), px1, py2, 0.0f).uv(uvx1, uvy2).color(255, 255, 255, 255).endVertex();
        bufferbuilder.vertex(ps.last().pose(), px2, py2, 0.0f).uv(uvx2, uvy2).color(255, 255, 255, 255).endVertex();
    ...
    @Override
    public void onClose() {
        dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(
