...
public final class ClientboundSableSnapshotDualPacket implements SableUDPPacket, SableTCPPacket {
    public static void encode(final FriendlyByteBuf buf, final ClientboundSableSnapshotDualPacket msg) {
        msg.encode(buf);
    }

    public static ClientboundSableSnapshotDualPacket decode(final FriendlyByteBuf buf) {
        return new ClientboundSableSnapshotDualPacket(buf);
    }
    ...
    private void handleClient(final Level level, final PacketReceiveMode packetReceiveMode) {
        ...
        for (final Entry entry : this.entries) {
            ...
            // [BUG-29] 收到运动快照即标记该子层级处于物理化飞行中，供增量更新补播操作反馈（声音+碎屑）。
            clientSubLevel.sable$markPhysicsSnapshot();
        }
    }
