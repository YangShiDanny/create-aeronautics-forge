...
public class ServerHandleHoldingHandler {
	...
	public static void sync() {
		final ClientboundPlayersHoldingHandlePacket pkt = new ClientboundPlayersHoldingHandlePacket(holdingPlayers.keySet());
		final FriendlyByteBuf buf = new FriendlyByteBuf(io.netty.buffer.Unpooled.buffer());
		ClientboundPlayersHoldingHandlePacket.CODEC.encode(buf, pkt);
		net.minecraftforge.network.PacketDistributor.ALL.noArg().send(new net.minecraft.network.protocol.game.ClientboundCustomPayloadPacket(pkt.type().id(), buf));
	}
}
