...
public class KeyWidget extends AbstractSimiWidget {
    ...
    public void renderWidget( final GuiGraphics graphics, final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (this.icon != null) {
            ...
            // [1.20.1 修复] 在按键表面渲染字符标签（原版源码缺失此功能）
            final Component keyLabel = this.keyName();
            final Font font = Minecraft.getInstance().font;
            final int labelWidth = font.width(keyLabel.getString());
            final float scale = 0.5f;
            final float labelX = this.getX() + (this.width - labelWidth * scale) / 2.0f;
            final float labelY = y + (14.0f - font.lineHeight * scale) / 2.0f;
            graphics.pose().pushPose();
            graphics.pose().translate(labelX, labelY, 0.0f);
            graphics.pose().scale(scale, scale, 1.0f);
            graphics.drawString(font, keyLabel.getString(), 0, 0, 0xFFFFFF, false);
    ...
    protected void renderHover( final GuiGraphics pGuiGraphics, final int pMouseX, final int pMouseY, final float pPartialTick) {
