...
public record ClientboundFinalizeSubLevelPacket(long plotCoordinate) implements SableTCPPacket {
    public static void encode(final FriendlyByteBuf buf, final ClientboundFinalizeSubLevelPacket msg) {
        msg.write(buf);
    }

    public static ClientboundFinalizeSubLevelPacket decode(final FriendlyByteBuf buf) {
        return ClientboundFinalizeSubLevelPacket.read(buf);
    }
