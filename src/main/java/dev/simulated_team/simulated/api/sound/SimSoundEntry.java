...
public record SimSoundEntry(ResourceLocation id, RegistryObject<SoundEvent> registryObject, SoundSource category) {
    ...
    public SoundEvent event() {
        // [1.20.1 port] Veil 把 SoundEvent 注册到 RegisterEvent 的 SOUND_EVENT 批次（晚于
        // 流体 BLOCK/FLUID 批次），registrate 的 .properties(sound) lambda 在更早批次立即取 SoundEvent
        // 时会因 registryObject.get() 未就位而 NPE。改为返回持有正确 id 的延迟创建实例，
        // 运行时按 id 查注册表播放，彻底绕过注册时序。NeoForge 1.21 下 Veil 注册更早，无此问题。
