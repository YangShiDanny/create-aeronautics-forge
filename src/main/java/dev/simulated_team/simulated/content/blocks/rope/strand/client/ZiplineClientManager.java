...
public class ZiplineClientManager implements InteractCallback {
     ...
     */
    public static  UUID raycastRope(final ClientLevelRopeManager ropeManager,
    ...
    private static void ridingTick() {
        ...
        if (AnimationTickHolder.getTicks() % 10 == 0)
            dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new RopeRidingPacket(ridingRope, false));
    }
    ...
    public static void disembark() {
        if (ridingRope != null)
            dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new RopeRidingPacket(ridingRope, true));
    ...
    public Result onUse(final int modifiers, final int action, final KeyMapping rightKey) {
        ...
        if (isDestroyer || (isWrench && mc.player.isShiftKeyDown())) {
            dev.simulated_team.simulated.network.SimPacketManager.INSTANCE.server().sendPacket(new RopeBreakPacket(hoveringRope));
            mc.player.swing(InteractionHand.MAIN_HAND);
            return new Result(true);
        }
