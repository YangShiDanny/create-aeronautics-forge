...
public class ServerSubLevel extends SubLevel implements PhysicsPipelineBody {
     ...
     */
    public Consumer<SableTCPPacket> playerSink() {
        return packet -> {
    ...
    public void setName( final String name) {
        if (!Objects.equals(name, this.getName())) {
            this.playerSink().accept(new ClientboundChangeSubLevelNamePacket(this.getUniqueId(), name));
        }
    ...
    public MassData getMassTracker() {
        // Lazily build the mass tracker if it hasn't been initialised yet. In the
        // NeoForge reference this is done eagerly by the physics system's
        // onSubLevelAdded observer, but on Forge the observer can occasionally
        // miss a freshly spawned sub-level (timing of container initialisation
        // vs. first allocation). Without this guard any consumer of the
        // tracker (processSubLevelRemovals, force application, assembly, ...) would
        // hit a NullPointerException the instant the sub-level is ticked.
        if (this.massTracker == null) {
            this.buildMassTracker();
        }
